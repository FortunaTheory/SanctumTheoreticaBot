...
export function createWhisperAdminCommand(scheduler: WhisperScheduler) {
  return {
    data: new SlashCommandBuilder()
      .setName("whisper-admin")
    ...
    async execute(interaction: ChatInputCommandInteraction) {
      if (!interaction.inGuild() || !interaction.member || !isAuthorizedMember(interaction.member)) {
        await interaction.reply({
          content: "Nur die Wächter dürfen die Stimme der Kuratorin lenken.",
