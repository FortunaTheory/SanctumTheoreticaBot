...
<style>
	...
	.entries { margin-top: 3rem; padding: 1.5rem; }.entry-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1px; margin-top: 1.2rem; background: var(--line); border: 1px solid var(--line); }.entry-list article { position: relative; min-height: 130px; padding: 1rem; background: var(--surface-sunken); }.entry-list p { margin: .75rem 0 0; line-height: 1.5; }.empty { color: var(--ink-muted); }details { margin-top: 1rem; } summary { display: inline-flex; gap: .35rem; align-items: center; cursor: pointer; font: .72rem var(--font-ui); }.edit-form { display: grid; gap: .7rem; margin-top: .8rem; }.edit-form label { font-size: .78rem; }.edit-form input, .edit-form textarea { width: 100%; padding: .5rem; border: 1px solid var(--line); background: var(--surface); color: var(--ink); font: inherit; }.edit-form textarea { min-height: 100px; }.edit-form button { padding: .55rem .7rem; }.delete-form { position: absolute; top: .8rem; right: .8rem; }.delete-form button { padding: .35rem; color: var(--danger); background: transparent; }@media (max-width: 760px) { .grid { grid-template-columns: 1fr; gap: 1rem; }.shell { margin-top: 2.5rem; } }
</style>