<script lang="ts">
	import { ArrowLeft, Eye, FileStack, Plus } from '@lucide/svelte';
...
<main>
	...
	<section class="shell">
		...
		<div class="grid">
			...
			<aside class="preview"><div class="section-label"><Eye size={16} /> Discord-Vorschau</div><div class="embed"><div class="embed-title">&#9672; ARCHIV-FRAGMENT · {title || 'Unbenannter Eintrag'}</div><div class="embed-body"><b>ARCHIV-FRAGMENT</b><blockquote>{text || 'Die Vorschau wartet auf einen Archivtext.'}</blockquote></div><div class="embed-footer">Archiv der verlorenen Dinge · gerade eben</div></div></aside>
		</div>
		<section class="entries"><div class="section-label"><FileStack size={16} /> Vorhandene Fragmente</div>{#if data.fragments.length === 0}<p class="empty">Noch keine Datenbankeinträge.</p>{:else}<div class="entry-list">{#each data.fragments as fragment}<article><strong>{fragment.title}</strong><p>{fragment.text}</p></article>{/each}</div>{/if}</section>
	</section>
</main>
