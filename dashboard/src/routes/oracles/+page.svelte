<script lang="ts">
	import { ArrowLeft, Eye, Pencil, Plus, Save, ScrollText, Trash2 } from '@lucide/svelte';
...
<main class="sanctum-page antialiased">
	...
	<section class="shell sanctum-shell">
		...
		<div class="grid">
			...
			<aside class="preview"><div class="section-label"><Eye size={16} /> Discord-Vorschau</div><div class="embed" style={`--accent: ${colors[aspect as keyof typeof colors]}`}><div class="embed-title">&#9672; ORAKELRESONANZ · {labels[aspect as keyof typeof labels]}</div><div class="embed-body"><blockquote>{text || 'Die Vorschau wartet auf eine Prophezeiung.'}</blockquote></div><div class="embed-footer">Die Kuratorin · gerade eben</div></div></aside>
		</div>
		<section class="entries"><div class="section-label"><ScrollText size={16} /> Vorhandene Orakel</div>{#if data.oracles.length === 0}<p class="empty">Noch keine Datenbankeinträge. Führe nach der Migration einmal <code>npm run seed</code> aus.</p>{:else}<div class="entry-list">{#each data.oracles as oracle}<article><span class="aspect {oracle.aspect}">{labels[oracle.aspect]}</span><p>{oracle.text}</p><details><summary><Pencil size={14} /> Bearbeiten</summary><form method="POST" action="?/update" enctype="multipart/form-data" class="edit-form"><input name="id" type="hidden" value={oracle.id} /><label>Aspekt<select name="aspect" value={oracle.aspect}><option value="archive">Hallen des Archivs</option><option value="lucid">Luzider Verfall</option><option value="enigma">Omegaprotokoll: Enigma</option></select></label><label>Prophezeiung<textarea name="text" maxlength="500">{oracle.text}</textarea></label><label>Neues Visual<input name="image" type="file" accept="image/jpeg,image/png,image/webp" /></label><button type="submit"><Save size={14} /> Aktualisieren</button></form></details><form method="POST" action="?/delete" class="delete-form"><input name="id" type="hidden" value={oracle.id} /><button type="submit" aria-label="Orakel löschen" title="Orakel löschen"><Trash2 size={15} /></button></form></article>{/each}</div>{/if}</section>
	</section>
</main>
...
<style>
	...
	select, textarea { width: 100%; border: 1px solid var(--line); border-radius: .25rem; padding: .75rem; color: var(--ink); background: var(--surface-sunken); font: inherit; }
	...
	.error { margin: 0; color: #ffc0d2; } .success { margin: 0; color: var(--success); }
	...
	.embed-title { font-weight: 700; font-size: .95rem; }.embed-body { margin-top: .8rem; font-size: .86rem; line-height: 1.5; }.embed-body blockquote { margin: .8rem 0; padding-left: .7rem; border-left: 2px solid #4e5058; font-style: italic; }.embed-footer { color: #949ba4; font-size: .72rem; }
	.entries { margin-top: 3rem; padding: 1.5rem; }.entry-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 1px; margin-top: 1.2rem; background: var(--line); border: 1px solid var(--line); }.entry-list article { position: relative; padding: 1rem; background: var(--surface-sunken); }.entry-list p { margin: .75rem 0 0; line-height: 1.5; }.aspect { color: var(--ink-soft); font: .68rem var(--font-ui); text-transform: uppercase; }.aspect.lucid { color: #70c5c9; }.aspect.enigma { color: #f08caf; }.empty { color: var(--ink-muted); } code { font-family: var(--font-ui); } details { margin-top: 1rem; } summary { display: inline-flex; gap: .35rem; align-items: center; cursor: pointer; font: .72rem var(--font-ui); }.edit-form { display: grid; gap: .7rem; margin-top: .8rem; }.edit-form label { font-size: .78rem; }.edit-form select, .edit-form textarea, .edit-form input { width: 100%; padding: .5rem; border: 1px solid var(--line); background: var(--surface); color: var(--ink); font: inherit; }.edit-form textarea { min-height: 100px; }.edit-form button { padding: .55rem .7rem; }.delete-form { position: absolute; top: .8rem; right: .8rem; }.delete-form button { padding: .35rem; color: var(--danger); background: transparent; }
